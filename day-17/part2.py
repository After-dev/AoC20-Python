...
def simulation(initial_state, cycles):
	l = len(initial_state)
	cubes_ON = [(x,y,0,0) for x,y in product(range(l), range(l)) if initial_state[x][y] == "#"]
					...
					for w in range(-cycle-1, cycle+2):
						...
						for dx,dy,dz,dw in product([-1,0,1],[-1,0,1],[-1,0,1],[-1,0,1]):
							if (dx != 0) or (dy != 0) or (dz != 0) or (dw != 0):
								if (x+dx, y+dy, z+dz, w+dw) in cubes_ON:
									n_cubes_ON += 1
						if ((x,y,z,w) not in cubes_ON) and (n_cubes_ON == 3):
							current_cubes_ON.append((x,y,z,w))
						if ((x,y,z,w) in cubes_ON) and (n_cubes_ON in [2,3]):
							current_cubes_ON.append((x,y,z,w))
